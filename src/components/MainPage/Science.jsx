...
export default function Science() {
  ...
  async function clickCategory(name) {
    if (token) {
      try {
        ...
        let level = levelResponse.data.data[0]
          ? +levelResponse.data.data[0].level
          : undefined;
        // Update user level
        let updateLevelResponse = await axios.put(
          `${API}/api/users/level?user_id=${u_id}&subject_id=${subject_id}&level=${level}`
        );
        // Update user subject
        let updateSubjectResponse = await axios.put(
          `${API}/api/users/subject?id=${u_id}&subject_id=${subject_id}`
        );
        // Check response statuses
        if (
          updateLevelResponse.status === 200 ||
          (updateLevelResponse.status === 201 &&
            updateSubjectResponse.status === 201)
        ) {
          window.localStorage.setItem("subjectName", name);
  ...
  const styl = token ? { maxWidth: "540px", cursor: "pointer" } : { maxWidth: "540px"}
